...
public class SportsTeamApp {
    ...
    private void calculateBattingAverage() {
        Stats stats = ui.askUserForBatsAndHits();
        BattingAverageCalculator calc = new BattingAverageCalculator();
        double battingAverage = 0;
        try {
            battingAverage = calc.calculateBattingAverage(stats);
            ui.printMessage("Batting average = " + battingAverage);

        } catch (BattingAverageException e) {
            ui.printMessage(e.getCustomMessage() + " atBats was:" +  e.getOriginalStatsObject().getAtBats() + " hits was: " +  e.getOriginalStatsObject().getHits());
